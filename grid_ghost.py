...
def build_grid_points(roi_geometry, count, fallback_bounds, spacing_deg=None):
    lon_low, lat_low, lon_up, lat_up = fallback_bounds
    if roi_geometry is not None:
        lon_low, lat_low, lon_up, lat_up = roi_geometry.bounds
    ...
    grid_points = make_points(n_rows, n_cols)
    expansion_steps = 0
...
def main():
    ...
    if args.extra_mode == "random":
        ...
        layout_info = {"grid_rows": None, "grid_cols": None, "grid_candidates": None, "grid_spacing_deg": None}
    else:
    ...
    print(
        ...
        f"spacing={layout_info['grid_spacing_deg']})"
    )
